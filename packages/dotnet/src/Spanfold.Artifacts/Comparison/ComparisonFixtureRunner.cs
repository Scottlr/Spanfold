...
/// <summary>
/// Validates and executes Spanfold contract fixtures using schema version 1.
/// </summary>
public static class ComparisonFixtureRunner
{
    /// <summary>
    ...
    public static ComparisonResult Run(JsonElement fixture)
    {
        ...
        if (liveHorizon.HasValue)
        {
            return builder.RunLive(liveHorizon.Value);
        }
    ...
    {
        var closedWindows = new List<ClosedWindow>();
        var openWindows = new List<OpenWindow>();
        foreach (var window in windows.EnumerateArray())
        {
            ...
            {
                openWindows.Add(new OpenWindow(
                    windowName,
                    key,
                    startPosition,
                    source,
            ...
            closedWindows.Add(new ClosedWindow(
                windowName,
                key,
                startPosition,
        ...
        return WindowHistory.FromRecords(closedWindows, openWindows);
    }

    private static ComparisonPlan CreatePlan(JsonElement plan)
    {
        var against = ReadAgainstSelectors(plan);
        var scopeWindow = plan.GetProperty("scopeWindow");
        var scope = ComparisonScope.All();
        if (scopeWindow.ValueKind != JsonValueKind.Null)
        {
            scope = ComparisonScope.Window(scopeWindow.GetString()!);
        }
    ...
    private static CohortActivity ReadCohortActivity(JsonElement cohort)
    {
        var activity = cohort.TryGetProperty("activity", out var activityElement)
            ...
            : "any";
        var count = ReadOptionalInt32(cohort, "count");
    ...
    private static TemporalPoint? ReadLiveHorizon(JsonElement plan)
    {
        ...
        return TemporalPoint.ForPosition(horizon.GetInt64());
    }
    ...
    private static IReadOnlyList<WindowSegment> ReadSegments(JsonElement window)
    {
        ...
        foreach (var segment in segments.EnumerateArray())
        {
            var parentName = ReadOptionalString(segment, "parentName");
